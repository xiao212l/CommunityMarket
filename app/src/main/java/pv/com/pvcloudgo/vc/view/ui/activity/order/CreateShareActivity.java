...
public class CreateShareActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void showData() {
        ...
        Log.w("qqqq11111111111111111",cartData.get(0));
    ...
    private void createShareBill() {
        ...
        if (mName.getText().toString().equals("请选择收货地址")) {
            ...
            mHttpHelper.Post(Contants.API.BASE_URL + "order/assemble/create/" + skuId + "/" + count + "/" +
                    AddressListActivity.addr.getId(), App.getInstance().getToken(), new SpotsCallBack<createShareBillBean>(this) {
                @Override
                public void onSuccess(Response response, createShareBillBean payMessageBean) {
                    if (payMessageBean != null && payMessageBean.getMessage().equals("拼单创建成功")) {
                        ToastUtils.show("拼单创建成功");
    ...
    public void next(Class cls, int data) {
        ...
        Intent intent = new Intent(CreateShareActivity.this, cls);
